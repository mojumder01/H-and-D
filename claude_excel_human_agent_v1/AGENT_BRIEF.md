...
| `No new Claude reply detected on the page.` | `ASSISTANT_SELECTORS` in `browser/claude_agent.py` don't match anything on the real page — this is the *safe* version of the bug below, now caught instead of silently mis-scraped | `browser/claude_agent.py` → `ASSISTANT_SELECTORS` |
| Output columns are filled with the literal prompt template (e.g. `Highlights HTML` = `<ul>\n<li>...</li>\n</ul>`, `Description HTML` ends with the `Rules:` block) marked `COMPLETED` | **Fixed — was the root cause of "doesn't wait after input".** The assistant-selectors matched nothing, so the old code fell back to `page.locator("body").inner_text()`. Since the prompt template itself contains the literal words `HIGHLIGHTS_HTML`/`DESCRIPTION_HTML`, the agent's own just-submitted (echoed) message satisfied the "response arrived" check instantly, before Claude ever replied. The rewritten `_assistant_text_at()` only reads from a confirmed *new* assistant-turn element (turn count must increase) and never falls back to whole-page text — if this still happens, `ASSISTANT_SELECTORS` needs updating against the real DOM. | `browser/claude_agent.py` |
...
`browser/claude_agent.py`'s selectors (`div[contenteditable="true"].ProseMirror`, `.font-claude-message`, `[data-testid="chat-message"]`, `button[aria-label="Send Message"]`) are best-guess from known Claude.ai UI patterns, not verified against a live DOM dump. A real run confirmed the two `ASSISTANT_SELECTORS` do not match anything on the live page (the agent was silently falling back to whole-page text, now removed — see the table above). If a row still errors with `No new Claude reply detected on the page.`, the fix is to update `ASSISTANT_SELECTORS` with the real class/attribute from Claude.ai's DOM: open DevTools (right-click a Claude reply → Inspect), copy the `outerHTML` of the smallest element that wraps just that one reply (not the whole conversation), and paste it back here.
