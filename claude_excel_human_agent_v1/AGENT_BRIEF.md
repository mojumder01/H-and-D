...
| `No new Claude reply detected on the page.` | `TURN_SELECTOR`/`RESPONSE_SELECTOR` in `browser/claude_agent.py` don't match anything on the real page (Claude.ai changed its DOM again) | `browser/claude_agent.py` |
...
The send button (`_send()` in `browser/claude_agent.py`) is still a guess — the DOM dump used to confirm the rest was captured while the composer was empty, and Claude's UI only shows a distinct send button once there's text (an empty composer shows a voice/mic button instead). If sending seems to silently do nothing, capture the composer's `outerHTML` **after typing something into it** and paste it back here so `_send()` can be corrected. Pressing Enter is the fallback and should work regardless (the input's `enterkeyhint="enter"` attribute is consistent with Enter-to-submit).
